...
public class HelloController implements Initializable {
    ...
    private ImageInfo positions = ImageInfo.builder()
            .positionZ(0)
    ...
    private void readFromFile() {
        Result res = ParserService.parse(Files.readAllLines(Paths.get("D:\\Programming\\University\\4 course\\AKG\\2\\my\\demo\\src\\main\\resources\\com\\example\\demo\\mclaren.obj")));
    ...
    private void draw() {
        ...
        var pixelBuffer = new PixelBuffer<>(WIDTH, HEIGHT, intBuffer, PixelFormat.getIntArgbPreInstance());
        ...
        Vector3D lightDirection = positions.getCameraTarget()
                .subtract(VertexService.convertToOrthogonal(positions.getCameraPosition(), positions.getCameraTarget()))
                .normalize();
        DrawerService.drawModel(transformedVertexes, transformedNormals, triangles, WIDTH, HEIGHT, zBuffer,
                lightDirection, pixels);
    ...
    private void handleKeyPress(KeyEvent event) {
        ...
        if (keysPressed.contains(KeyCode.W)) {
            ...
            positions.setCameraTarget(new Vector3D(positions.getCameraTarget().getX(),
        ...
        if (keysPressed.contains(KeyCode.A)) {
            ...
            positions.setCameraTarget(new Vector3D(positions.getCameraTarget().getX() + MOVE_SPEED,
                    positions.getCameraTarget().getY(),
        ...
        if (keysPressed.contains(KeyCode.N)) {
            positions.setCameraPosition(new Vector3D(positions.getCameraPosition().getX() + MOVE_SPEED,
                    positions.getCameraTarget().getY(),
                    positions.getCameraTarget().getZ()));
        }
