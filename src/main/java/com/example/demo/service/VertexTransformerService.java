...
public class VertexTransformerService {
    private static final float FOV = 60; // 3.14 / 8???
    private static final float NEAR_PLANE_DISTANCE = 0.1f; // TODO RENAME
    private static final float FAR_PLANE_DISTANCE = 100; // TODO RENAME

    public static Vector3D toOrthogonal(Vector3D v, Vector3D rotationPoint) {
        var r = v.getX();
        var phi = v.getY();
        var zenith = v.getZ();
    ...
    public static void transformVertexes(ImageInfo info, double gridWidth, double gridHeight, Vector4D[] res) {
        float xMin = 0;
        float yMin = 0;
        float minDepth = 0;
        float maxDepth = 1;
        ...
        var rotationMatrix = Matrix4x4.createRotationX(info.getPositionX())
                .multiply(Matrix4x4.createRotationY(info.getRotationY()));
        var scaleMatrix = Matrix4x4.createScale(0.01, 0.01, 0.01);

        var viewMatrix = Matrix4x4.createView(toOrthogonal(info.getCameraPosition(), info.getCameraTarget()),
